...
QSqlQuery WarehouseRepository::getTransferLog(int shopId, const QString& from, const QString& to)
{
    return db().execQuery(QString(
...
{
    auto q = prepare("SELECT replenish_micro_warehouse(:shop, :user, :fill)");
    q.bindValue(":shop", shopId);
    q.bindValue(":user", userId);
    q.bindValue(":fill", fillTo);
